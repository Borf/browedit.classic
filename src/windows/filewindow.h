...
class cFileWindow : public cWindow
{
	...
	public:
		...
		cOkButton(cWindow* parent, void (*pCallback)(std::string), TiXmlDocument &skin) : cWindowButton(parent,skin)
		{
			callback = pCallback;
...
public:
	...
	cFileWindow(cTexture* t, cFont* f, void (*pCallback)(std::string), TiXmlDocument &skin) : cWindow(t,f,skin)
	{
		windowType = WT_FILE;
		resizable = true;
		visible = true;
